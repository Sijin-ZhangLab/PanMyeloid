...
### without 'stage' for CRC !!!!!!!!!!!!!!!!!
DoSurvivalPlot(cancer = 'CRC', genelist, normalize.by.gene = "PTPRC", low.quantile = 0.4, high.quantile = 0.6, column.used = c("gender",  "age"), survival.used = "OS", signature.name = "GeneSet")

DoSurvivalPlot(cancer = 'THCA', genelist, normalize.by.gene = "PTPRC", low.quantile = 0.4, high.quantile = 0.6, column.used = c("gender"),  survival.used = "OS", signature.name = "GeneSet")
...
plt=ggplot()+theme_minimal()+
  ...
  plt=plt+scale_fill_gradient2(
    ...
    midpoint = 0, name="log10(HR)",limit=c(-0.5,0.5)
  )
...
genelist <- c("SPP1")
cancerType <- 'PAAD'
DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "PTPRC", low.quantile = 0.4, high.quantile = 0.6, column.used = c("gender", "stage", "age"), survival.used = "OS", signature.name = "GeneSet")
...
genelist <- c("SPP1")
cancerType <- 'CRC'
DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "PTPRC", low.quantile = 0.4, high.quantile = 0.6, column.used = c("gender",  "age"), survival.used = "OS", signature.name = "GeneSet")

genelist <- c("SPP1")
cancerType <- 'OV'
DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "PTPRC", low.quantile = 0.4, high.quantile = 0.6, column.used = c("age"), survival.used = "OS", signature.name = "GeneSet")


### other angiogenesis associated marcophage dataset
genelist <- c("FN1")
cancerType <- 'KIDNEY'
res <- DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "", low.quantile = 0.4, high.quantile = 0.6, column.used = c("gender", "stage", "age"), survival.used = "OS", signature.name = "GeneSet")
res$survplot

genelist <- c("MARCO")
cancerType <- 'OV'
res <- DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "PTPRC", low.quantile = 0.3, high.quantile = 0.7, column.used = c( "age"), survival.used = "OS", signature.name = "GeneSet")
res$survplot

genelist <- c("IDO1")
cancerType <- 'ESCA'
res <- DoSurvivalPlot(cancer = cancerType, genelist, normalize.by.gene = "", low.quantile = 0.4, high.quantile = 0.6, column.used = c("age"), survival.used = "OS", signature.name = "GeneSet")
res$survplot
